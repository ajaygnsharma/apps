...
async def snmp_get(oid, ip, community, version):
    """
    Asynchronous SNMP GET request using pysnmp 7.1.16 with MIB support.
    """
    try:
        ...
        # Enable human-readable MIB lookup
        compiler.add_mib_compiler(mib_builder);
        mib_builder.add_mib_sources(builder.DirMibSource('/usr/share/snmp/mibs'))
        object_identity = ObjectIdentity(oid).resolveWithMib(mib_view_controller)
        ...
        if error_indication:
            ...
            return f"{var_binds[0][0].prettyPrint()} = {var_binds[0][1]}"
