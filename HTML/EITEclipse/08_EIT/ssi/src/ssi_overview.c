...
/*------------------------------------------------------------------------
 Function:    ssi_pswd_msg()
 Description: SSI: Display if correct/incorrect password
 Input:       None
 Output:      String the message
------------------------------------------------------------------------*/
word16 ssi_pswd_msg(byte **str_)
cmx_reentrant 
{  
  char pswd_msg[50];

  if(flag.password_entered_incorrect == TRUE)
  {
    strcpy(pswd_msg,"<h2 style=\"color:red\"> Incorrect Password! </h2>");   /* Enter a msg that password is invalid */
    *str_=(byte *)pswd_msg;
  }
  else
  {
	strcpy(pswd_msg," ");                                /* No need to enter a message */
    *str_=(byte *)pswd_msg;
  }

  flag.password_entered_incorrect = FALSE;

  return (strlen((char *)pswd_msg));
}
...
cmx_reentrant 
{
  ...
  switch(iserver.model)
  {
    ...
    default:         strcpy(model_buf,"Error"); break;
  }
  
  *str_=(byte *)model_buf;           
  return (strlen((char *)model_buf));
}
...
cmx_reentrant 
{
  ...
  switch(serial.parity)
  {
    ...
    case 2: strcat(str,",E"); break;
    case 3: strcat(str,",O"); break;
