...
/*------------------------------------------------------------------------
 Function:    ssi_login_msg()
 Description: SSI: Display if correct/incorrect password
 Input:       None
 Output:      String the message
------------------------------------------------------------------------*/
word16 ssi_login_msg(byte **str_)
cmx_reentrant 
{  
  char pswd_msg[200];

  if(flag.confirm_pswd_entered_incorrect == TRUE)
  {
    strcpy(pswd_msg,"<h2 style=\"color:red\"> ERROR. New and Confirm Password must match. </h2>");   
	/* Enter a msg that password is invalid */
    *str_=(byte *)pswd_msg;
  }
  else if(flag.confirm_pswd_entered_incorrect == 2)
  {
	strcpy(pswd_msg,"<h2 style=\"color:red\">New Password Saved! </h2>");
    *str_=(byte *)pswd_msg;
  }
  else
  {
...
/*------------------------------------------------------------------------
 ...
 Output:      String the message
------------------------------------------------------------------------*/
word16 ssi_admin_msg(byte **str_)
cmx_reentrant 
{  
  char pswd_msg[200];
...
/*------------------------------------------------------------------------
 Function:    ssi_serial_msg()
 Description: SSI: Display if correct/incorrect password
 Input:       None
 Output:      String the message
------------------------------------------------------------------------*/
word16 ssi_serial_msg(byte **str_)
cmx_reentrant 
{  
  char pswd_msg[200];
